...
public class TwilioResource
{
    ...
    @POST
    @Path("modal")
    public void postModalMessage(@FormParam("message") String message, @FormParam("to") String to,
    ...
    @POST
    @Path("confirmation")
    public void postConfirmation(@FormParam("From") String from, @FormParam("Body") String body)
    {
        ...
        if (code != null)
        {
            ...
            {
                System.out.println("Searching for confirmation with phone=" + phone + " and code=" + code);

                // Attempt to confirm
                Confirmation c = confirmationSvc.confirm(phone, code);
                if (c != null)
                {
                    System.out.println("TWILIO: Ready to confirm and start payment process!");
                ...
                {
                    System.out.println("Was not able to confirm with phone=" + phone + " and code=" + code);
            ...
            { // else five digit phone verification
                System.out.println("Phone Verification: confirmation with phone=" + phone + " and code=" + code);
                ...
                {
                    System.out.println("Dollar.ly not able to verify with phone=" + phone + " and code=" + code);
                    pc = phoneConfirmationSvc.getByPhoneCode(phone, code);
                    if (pc == null)
                    {
                        phoneConfirmationSvc.createAndSendFailureMessage(phone, code, "PHONE_CONF_INVALID_CODE");
                    } else if (pc.getStatus() == PhoneConfirmation.Status.EXPIRED)
                    {
                        phoneConfirmationSvc.createAndSendFailureMessage(phone, code, "PHONE_CONF_EXPIRED_CODE");
                    }
                }
            }
        } else
    ...
    @POST
    @Path("status/{orderId}")
    public void postStatus(@PathParam("orderId") String orderId, @FormParam("SmsStatus") String smsStatus)
    {
    ...
    private void postPhoneConfirmation(String userId, Long phone) throws Exception
    {
        DefaultHttpClient httpClient = new DefaultHttpClient();
        Credentials credentials = new UsernamePasswordCredentials("hamilton", "Wi11u|$hit53$Bs4m3");
        httpClient.getCredentialsProvider().setCredentials(org.apache.http.auth.AuthScope.ANY, credentials);
        ClientExecutor clientExecutor = new ApacheHttpClient4Executor(httpClient);
        ClientRequestFactory fac = new ClientRequestFactory(clientExecutor, new URI(userBaseUrl));
        ClientRequest request = fac.createRequest(phoneVerifyURL + userId + "/" + phone);
        ClientResponse cr = request.post();

        System.out.println("Phone verification response status code: " + cr.getStatus());
 ...
 * @POST
 * 
 * @Path("status/phone/{userId}") public void
 * postPhoneStatus(@PathParam("userId") String userId,
 * 
 * @FormParam("SmsStatus") String smsStatus) {
 * 
 * // Validate this is not a duplicated request from Twilio PhoneConfirmation pc
 * = phoneConfirmationSvc.getByUserId(userId); if (pc.getStatus() !=
 * PhoneConfirmation.Status.REQUESTED) { System.out
 * .println("Dupe phone verification status update from Twilio. " + userId);
 * return; }
 * 
 * if (smsStatus.equals("sent")) {
 * System.out.println("Got Phone Verification SMS from Twilio: " + smsStatus);
 * 
 * pc.setStatus(PhoneConfirmation.Status.DELIVERED); } else {
 * pc.setStatus(PhoneConfirmation.Status.UNDELIVERABLE); //
 * p.fail(FailReason.UNCONFIRMABLE); // transactionSvc.update(p); }
 * 
 * pc.setStatusLastModifiedOn(new Date()); phoneConfirmationSvc.update(pc); }
 */